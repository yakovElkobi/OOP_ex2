    ...
    private static void moveAgants(game_service game, directed_weighted_graph graph) {
        ...
        for (int i = 0; i < agents.size(); i++) {
            ...
            for (CL_Pokemon p : listOfPokemons) {
                double d = algoGraph.shortestPathDist(src, p.getEdge().getSrc());
            ...
            int dest = -1;
            if (min == 0) dest = algoGraph.shortestPath(src, min_node.getDest()).get(1).getKey();
            else dest = algoGraph.shortestPath(src, min_node.getSrc()).get(1).getKey();
//            dest = nextNode(gameGraph,pokemon.get_edge().getDest(), src);
    ...
    private static int nextNode(dw_graph_algorithms algoGraph, int pokemon, int src) {
      //  dw_graph_algorithms gameGraphAlgo = new DW_Graph_Algo();
        // gameGraphAlgo.init(algoGraph);

        List<node_data> neighbors = algoGraph.shortestPath(src, pokemon);

        Iterator<node_data> itr = neighbors.iterator();
        node_data nextNode = null;
        for (int i = 0; i < neighbors.size() || i < 2; i++) {
            nextNode = itr.next();
        }
    ...
    private void init(game_service game) {
        ...
        gameWindow = new GameFrame(); // gui
        gameWindow.update(arena); // gui
        gameWindow.setVisible(true); // gui
